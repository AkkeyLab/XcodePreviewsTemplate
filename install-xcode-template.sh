...
TAPPLE_COMPONENTS_TEMPLATE_DIR="$XCODE_TEMPLATE_DIR""/Swift UIKit Class.xctemplate"
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

# Copy SwiftCocoaClass templates into the local SwiftCocoaClass template directory
xcodeTemplate () {
  echo "==> Copying up Tapple Xcode file template..."
...
echo "==> ... success!"
echo "==> Tapple have been set up. In Xcode, select 'New File...' to use AkkeyLab templates."
